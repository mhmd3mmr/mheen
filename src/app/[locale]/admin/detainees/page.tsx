      ...
      <FadeUp>
        <div>
          <h1 className="font-qomra text-2xl font-semibold text-primary md:text-3xl">
            {tAdmin("sidebar.detainees")}
          </h1>
          <p className="mt-2 text-foreground/80">
            توثيق بيانات المعتقلين والمفقودين من أبناء مهين.
            {pendingCount > 0 && (
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rows.map((d) => (
                <tr
                  key={d.id}
                  className={`border-t border-primary/10 hover:bg-primary/5 ${d.status === "pending" ? "bg-amber-50/50" : ""}`}
                >
                  <td className="px-4 py-3 align-top">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-foreground/60">{d.name_en}</div>
                        {d.submitted_by && (
                          <div className="text-xs text-foreground/40">
                            أرسله: {d.submitted_by}
                          </div>
                        )}
                      </div>
                    </div>
