...
type Props = {
  initialDetainees: Detainee[];
  locale: string;
};

export function DetaineesClient({ initialDetainees, locale }: Props) {
  const t = useTranslations("pages.detainees");
  ...
  async function handleDetaineeSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
  ...
  const years = useMemo(() => {
    ...
    for (const d of initialDetainees) {
      if (d.arrest_date) {
        const y = d.arrest_date.slice(0, 4);
        if (y) set.add(y);
      }
    }
  ...
  const filtered = useMemo(() => {
    return initialDetainees.filter((d) => {
      ...
      const matchesYear =
        !selectedYear || (d.arrest_date && d.arrest_date.startsWith(selectedYear));
      return matchesSearch && matchesYear;
    });
  }, [initialDetainees, searchQuery, selectedYear]);
  ...
  return (
    <div className="min-h-[60vh] overflow-x-hidden pb-24">
      {/* Hero */}
      <section className="relative overflow-hidden bg-primary px-4 pb-28 pt-16 md:-mx-8 md:-mt-8 md:px-8 md:pb-32 md:pt-24">
          ...
          >
            <div className="mt-8 rounded-2xl border border-primary/10 bg-background p-6 shadow-lg">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h2 className="font-qomra text-lg font-semibold text-primary">{t("addDetainee")}</h2>
                  <p className="text-xs text-foreground/60">{t("addDetaineeDesc")}</p>
                </div>
              ...
              {formSuccess ? (
                ...
                <form ref={formRef} onSubmit={handleDetaineeSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-foreground/70">{t("arrestDateInput")}</label>
                    <input type="date" name="arrest_date" className="w-full rounded-lg border border-primary/15 bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-foreground/70">{t("statusArInput")}</label>
                    <textarea name="status_ar" rows={2} className="w-full rounded-lg border border-primary/15 bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-foreground/70">{t("statusEnInput")}</label>
                    <textarea name="status_en" rows={2} className="w-full rounded-lg border border-primary/15 bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                  </div>
                  <div>
...
function DetaineeCard({
  ...
  detainee: Props["initialDetainees"][number];
  isAr: boolean;
  index: number;
  arrestLabel: string;
  statusLabel: string;
  ...
  const status = isAr ? (d.status_ar || d.status_en) : (d.status_en || d.status_ar);
      ...
      <div className="flex flex-1 flex-col justify-center gap-1.5 px-4 py-3">
        {d.arrest_date && (
          <div className="flex items-center gap-2 text-xs text-foreground/60">
            ...
            <span>
              {arrestLabel}: {d.arrest_date}
            </span>
          </div>
        )}
        {status && (
          <div className="flex items-center gap-2 text-xs text-foreground/60">
            <ShieldAlert className="h-3.5 w-3.5 shrink-0 text-primary/50" />
